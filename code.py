...
time_textarea = label.Label(
    big_font, color=time_color, x=time_position[0], y=time_position[1]
)

wakeup_time_color = 0xFFFFFF
wakeup_time_position = (15, 200)
wakeup_time_textarea = label.Label(
    info_font,
...
light_on_time_textarea = label.Label(
    info_font,
    color=light_on_time_color,
    x=light_on_time_position[0],
    y=light_on_time_position[1],
)
...
def whichDay():
    ...
    current_day = now[6]
    now_mk = time.mktime(
        (now[0], now[1], now[2], now[3], now[4], now[5], now[6], now[7], now[8])
    )
        ...
        # set wake up time to the next day's wake up time after current day's wake up time
            ...
            if current_day == 6:
                ...
                if current_day == day:
                    ...
                    use_day = day + 1
    input_wake_up_time_text = (
        "Wake up " + days_str[use_day] + " at " + input_wake_up_time
    )
...
def subtract30min(day):  # subtract 30 min
    ...
    light_on_time_textarea.text = "Light starting at: " + formatTime(
        hour_minus30, minutes_minus30
    )
